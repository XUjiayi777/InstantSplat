...
def main(source_path, model_path, ckpt_path, device, batch_size, image_size, schedule, lr, niter, 
    ...
    if depth_thre > 0:
        def print_type_and_shape(name, var):
            if hasattr(var, 'shape'):
                print(f"{name}: type={type(var)}, shape={var.shape}")
            else:
                print(f"{name}: type={type(var)}")

        print_type_and_shape("sorted_conf_indices", sorted_conf_indices)
        print_type_and_shape("depthmaps", depthmaps)
        print_type_and_shape("pts3d", pts3d)
        print_type_and_shape("intrinsics", intrinsics)
